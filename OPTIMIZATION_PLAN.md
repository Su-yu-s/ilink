...
|--------|------|------|-------------|----------|----------|
| P0 | 安全加固 | 2 | ~50 行 | **低** | 安全问题必须最先解决，改动能小且高度可控 |
| P1 | CSS 债务清偿 | 5 | ~5,500 行 (70%) | **中** | 项目最大单一债务，越晚越难改；视觉回归可截图对比 |
| P2 | Java 分层精简 | 6 | ~1,200 行 (25%) | **中高** | 涉及核心业务逻辑，需要在有测试防护后进行 |
| P3 | JS 模块化 | 5 | ~800 行 (40%) | **中** | 依赖 P2 的 API 路径收敛，前后端需要协同 |
| P4 | 数据与测试健全 | 4 | ~400 行 SQL | **中** | 为后续重构提供安全网 |
| P5 | 配置清理与依赖升级 | 4 | ~50 行 | **中高** | Spring Boot 大版本升级风险最高，放最后 |
...
| 维度 | 重构前 | 重构后 | 改善比例 |
|------|--------|--------|----------|
| style.css 行数 | 7,785 | ~1,100 | **-86%** |
| CSS `!important` 数 | 2,165 | <200 | **-91%** |
| CSS 文件数 | 20 | 18（合并重复、删除备份） | -10% |
| common.js 行数 | 1,331 | ~420 | **-68%** |
| JS 重复函数 | 30+ | 0 | **-100%** |
| Controller 平均行数 | ~300 | ~150 | **-50%** |
| Service 接口数 | 20 | 8（删除纯空接口） | **-60%** |
| SQL 注入模式 | 8 | 0 | **-100%** |
| 认证体系 | 双轨 | Spring Security 统一 | 标准化 |
| 测试文件数 | 7 | 12+ | +70% |
| Java 总代码行数 | ~12,000 | ~10,500 | **-12.5%** |
