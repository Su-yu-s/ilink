...
- 全量 Maven 测试：27 个测试套件、113 个测试，0 失败、0 错误、0 跳过。
- 现有 MySQL 5.7 数据库从 V15/V22 历史状态兼容升级至 V23；空库从 V0.5 连续执行 24 个迁移至 V23，均无失败记录。
- Playwright 发布回归覆盖 7 个公开入口、4 类公开详情、竞赛 API、教师注册与资料自动公开、393/768/1280 三档无页面级溢出、44px 触控目标、安全登录回跳、记住我、管理员竞赛后台、密码重置枚举防护和临时账号级联清理。
- 回归中额外发现并修复 baseline 旧库缺少任务评论字段/里程碑表，以及管理员删除后的导师详情缓存残留。
- 已删除 2 个可确认由旧初始化器生成的演示导师；5 个来源不明的疑似测试账号仅保留在审计报告中，未自动删除。
- 聊天已读状态和已读回执按产品决定不实现，相关空能力不列为遗留项。
...
- 反馈动作白名单并校验日志归属。
- 后续产品调整已移除组队大厅的个性化队伍推荐区和对应接口。
- 团队空间保留队长候选人推荐；推荐失败时隐藏该区域，不阻断工作台。
- 增加权限、空结果和回退测试。
